...
void MRF::MRFProcess()
{
    ...
    //                     if(neighbor_depth[0] < max_depth_ && gray_diference[0] < 4) flag[0] =1;
    //                     if(neighbor_depth[1] < max_depth_ && gray_diference[1] < 4) flag[1] =1;
    //                     if(neighbor_depth[2] < max_depth_ && gray_diference[2] < 4) flag[2] =1;
    //                     if(neighbor_depth[3] < max_depth_ && gray_diference[3] < 4) flag[3] =1;
    //                     for(int k = 0; k < 4; k++)
    //                     {
    //                         // if(flag[k] == 1 && max_depth_ > neighbor_depth[k])
    //                         // {
    //                             // sum = sum + neighbor_depth[k] * (4 - gray_diference[k]);
    //                             // cnt = cnt + (4 - gray_diference[k]);
    //                             // count ++;
    //                         // }
    //                         if(flag[k] == 1 && mindepth > neighbor_depth[k])
    //                         {
    //                             mindepth = neighbor_depth[k];
    //                             count ++;
    //                         }
    //                     }
    //                     if(count > 0)
    ...
    // cv::Mat M(raw_image_.rows, raw_image_.cols, CV_32F);//把点投影到M上
    // cv::Mat P(raw_image_.rows, raw_image_.cols, CV_32F);//扩展投影点
    // M = depth_image_.clone();
    // P = depth_image_.clone();
    ...
    //     {
    //         if(depth_image_.at<float>(i,j) != INF)
    //             temp_1.at<char>(i,j) =  depth_image_.at<float>(i,j)/max_depth_ *255;
    //         else
    //             temp_1.at<char>(i,j) =255;
    //     }
    // cv::imshow("depthmap_1", temp_1);
    // cv::imwrite("projeciton.png",temp_1);
    // cv::waitKey(0);
    ...
    for(int i = 0; i < raw_image_.rows; i++)
        for(int j = 0; j < raw_image_.cols; j++)
        {
            if(depth_image_.at<float>(i,j) != INF)
                ...
                depth_image_.at<float>(i,j) = 255;
        }
    
    cv::Rect rect_1(6, 150, 600, 200);
    cv::Rect rect_2(607, 150, 600, 200);
    ...
    result_ = depth_image_;
    for(int i = 0; i < result_.rows; i++)
        for(int j = 0; j < result_.cols; j++)
        {
            result_.at<float>(i,j) = INF;
            if(i >= 150 && i < result_.rows - 25 && j >= 6 && j < result_.cols - 36)
                result_.at<float>(i,j) = result.at<float>(i-150,j-6)/255*max_depth_;
        }
    //  show temp result (depth map)
    // cv::Mat temp_2(result.rows, result.cols, CV_8U);//把点投影到M上
    // for(int i = 0; i < result.rows; i++)
    //     for(int j = 0; j < result.cols; j++)
    //     {
    //         if(result.at<float>(i,j) != INF)
    //             temp_2.at<char>(i,j) =  result.at<float>(i,j);
    //         else 
    //             temp_2.at<char>(i,j) =255;
    //     }
    // cv::imshow("depthmap_2", temp_2);
    // cv::imwrite("only_linear.png",temp_2);
    // cv::waitKey(0);
    // cv::destroyWindow("depthmap_2");
    ...
    for(int i = 0; i < result.rows; i ++)
    {
        for(int j = 0; j < result.cols; j++)
        {
            if(result.at<float>(i,j) < 160)
            {
                pcl::PointXYZ point;
                point.z = result.at<float>(i,j)/255*max_depth_;
                point.x = (j+6) * point.z;
                point.y = (i+150) * point.z;
    ...
    for(int i = 0; i < result.rows; i ++)
    {
        for(int j = 0; j < result.cols; j++)
        {
            if(result.at<float>(i,j) < 160)
            {
                ...
                point.r = raw_image_.at<cv::Vec3b>(i+150,j+6)[2];//round() most closed number
                point.g = raw_image_.at<cv::Vec3b>(i+150,j+6)[1];
                point.b = raw_image_.at<cv::Vec3b>(i+150,j+6)[0];
    ...
    // pcl::transformPointCloud (*result_cloud_rgb, *result_cloud_rgb, calibration_->rt2_);
    cout << calibration_->Position_ << endl;
    pcl::transformPointCloud (*result_cloud_rgb, *result_cloud_rgb, calibration_->Position_);//地图坐标系
    // pcl::transformPointCloud (*result_cloud_rgb, *result_cloud_rgb, calibration_->Rt_.inverse());//激光雷达坐标系
